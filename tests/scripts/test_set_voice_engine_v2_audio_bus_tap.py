...
from scripts.set_voice_engine_v2_audio_bus_tap import (
    DEFAULT_TAP_MAX_DURATION_SECONDS,
    disable_audio_bus_tap,
    enable_audio_bus_tap,
    load_settings,
    main,
...
def test_status_reports_safe_when_pre_stt_shadow_is_enabled(
    tmp_path: Path,
) -> None:
    ...
    assert status.safe_to_enable_audio_bus_tap is True
    assert status.reason == "safe"
    assert status.pre_stt_shadow_enabled is True
    assert status.faster_whisper_audio_bus_tap_enabled is False


def test_enable_audio_bus_tap_sets_only_guarded_fields(tmp_path: Path) -> None:
    settings_path = tmp_path / "settings.json"
    _write_settings(settings_path)
...
def test_enable_audio_bus_tap_creates_backup_by_default(tmp_path: Path) -> None:
    settings_path = tmp_path / "settings.json"
    _write_settings(settings_path)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "runtime_candidates_enabled_must_remain_false_for_audio_bus_tap",
        ),
        (
            {"pre_stt_shadow_enabled": False},
            "pre_stt_shadow_enabled_must_be_true_for_audio_bus_tap_validation",
        ),
    ],
)
def test_enable_audio_bus_tap_refuses_unsafe_states(
    tmp_path: Path,
    ...
    settings = load_settings(settings_path)
    assert settings["voice_engine"]["faster_whisper_audio_bus_tap_enabled"] is False


def test_disable_audio_bus_tap_preserves_duration_setting(tmp_path: Path) -> None:
    settings_path = tmp_path / "settings.json"
    _write_settings(
        settings_path,
        voice_engine_overrides={
            "faster_whisper_audio_bus_tap_enabled": True,
            "faster_whisper_audio_bus_tap_max_duration_seconds": 2.0,
        },
    )
    ...
    assert voice_engine["faster_whisper_audio_bus_tap_max_duration_seconds"] == 2.0
...
def test_cli_status_returns_success(
    ...
    assert payload["status"]["safe_to_enable_audio_bus_tap"] is True
...
def test_cli_enable_refuses_when_pre_stt_shadow_is_disabled(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    settings_path = tmp_path / "settings.json"
    _write_settings(
        settings_path,
        voice_engine_overrides={"pre_stt_shadow_enabled": False},
    )

...
from scripts.set_voice_engine_v2_runtime_candidates import (
    DEFAULT_RUNTIME_CANDIDATE_ALLOWLIST,
    disable_runtime_candidates,
    enable_runtime_candidates,
    load_settings,
    main,
...
def test_status_reports_safe_to_enable_runtime_candidates(tmp_path: Path) -> None:
    ...
    assert status.safe_to_enable_runtime_candidates is True
    assert status.reason == "safe"
    assert status.runtime_candidates_enabled is False
    assert status.runtime_candidate_intent_allowlist == DEFAULT_RUNTIME_CANDIDATE_ALLOWLIST


def test_enable_runtime_candidates_sets_only_guarded_candidate_fields(
    tmp_path: Path,
) -> None:
    settings_path = tmp_path / "settings.json"
    _write_settings(settings_path)
    ...
    assert voice_engine["runtime_candidates_enabled"] is True
    assert voice_engine["runtime_candidate_intent_allowlist"] == [
        "assistant.identity",
        "system.current_time",
    ]


def test_enable_runtime_candidates_creates_backup_by_default(tmp_path: Path) -> None:
    settings_path = tmp_path / "settings.json"
    _write_settings(settings_path)
...
def test_enable_runtime_candidates_refuses_unsafe_states(
    tmp_path: Path,
    ...
    settings = load_settings(settings_path)
    assert settings["voice_engine"]["runtime_candidates_enabled"] is False


def test_disable_runtime_candidates_preserves_allowlist(tmp_path: Path) -> None:
    settings_path = tmp_path / "settings.json"
    _write_settings(
        settings_path,
...
def test_cli_status_returns_success(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    settings_path = tmp_path / "settings.json"
    _write_settings(settings_path)
    ...
    assert payload["status"]["safe_to_enable_runtime_candidates"] is True
...
def test_cli_enable_and_disable_return_success(
    ...
    assert enable_output["status"]["voice_engine.runtime_candidates_enabled"] is True

    assert disable_exit == 0
    assert disable_output["ok"] is True
    assert disable_output["status"]["voice_engine.runtime_candidates_enabled"] is False
...
def test_cli_enable_refuses_unsafe_state(
    ...
    assert "voice_engine_enabled_must_remain_false" in payload["message"]
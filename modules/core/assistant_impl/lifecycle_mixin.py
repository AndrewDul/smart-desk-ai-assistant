...
from modules.runtime.backend_autostart import BackendAutostartService  # NEXA_BACKEND_AUTOSTART_IMPORT
...
class CoreAssistantLifecycleMixin:
    ...
    def _start_vision_backend(self) -> None:
        ...
        try:
            ...
            if isinstance(status, dict) and bool(status.get("lazy_start", False)):
                append_log("Vision backend lazy-start is enabled; skipping boot camera start.")
                return
    ...
    def shutdown(self) -> None:
        ...
        # NEXA_LOOK_AT_ME_SHUTDOWN
        try:
            session = getattr(self, "look_at_me_session", None)
            if session is not None:
                session.close()
        except Exception as error:
            log_exception("Failed to close look-at-me session during shutdown", error)

...
from modules.system.utils import MEMORY_PATH, append_log, load_json, save_json
...
class SimpleMemory:
    ...
    def recall(self, key: str) -> str | None:
        ...
        matched_key = self._find_matching_storage_key(memory_data, key)
        if matched_key is None:
            return None

        if self._clean_text(key) != matched_key:
            append_log(f"Memory recall matched '{self._clean_text(key)}' -> '{matched_key}'")

        return memory_data[matched_key]
    ...
    def forget(self, key: str) -> tuple[str | None, str | None]:
        ...
        removed_value = memory_data.pop(matched_key, None)
        save_json(self.path, memory_data)

        append_log(f"Memory deleted: {matched_key}")
        return matched_key, removed_value
    ...
    def _find_matching_storage_key(self, memory_data: dict[str, str], query: str) -> str | None:
        ...
        best_key = self._find_best_match(memory_data, query_clean)

...
DEFAULT_PRE_STT_SHADOW_LOG_PATH = "var/data/voice_engine_v2_pre_stt_shadow.jsonl"


@dataclass(frozen=True, slots=True)
class PreSttShadowSafetyStatus:
    """Safety summary for the Stage 21B pre-STT shadow switch."""
    ...
    safe_to_enable_pre_stt_shadow: bool
    reason: str
    ...
    def to_json_dict(self) -> dict[str, Any]:
        return {
            ...
            "voice_engine.pre_stt_shadow_log_path": self.pre_stt_shadow_log_path,
            "safe_to_enable_pre_stt_shadow": self.safe_to_enable_pre_stt_shadow,
            "reason": self.reason,
        }
...
def status_from_settings(
    *,
    ...
    if enabled:
        ...
        reason = "runtime_candidates_enabled_must_remain_false_for_isolated_pre_stt_shadow"

    return PreSttShadowSafetyStatus(
        ...
        pre_stt_shadow_log_path=pre_stt_shadow_log_path,
        safe_to_enable_pre_stt_shadow=safe_to_enable,
        reason=reason,
    )


def enable_pre_stt_shadow(
    *,
    settings_path: Path,
    ...
    if not current_status.safe_to_enable_pre_stt_shadow:
        raise RuntimeError(
            "Refusing to enable pre-STT shadow: "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Safely inspect, enable or disable the Voice Engine v2 pre-STT "
            "shadow hook."
        )
    )

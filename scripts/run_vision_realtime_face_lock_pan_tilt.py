...
DEFAULT_STATUS_PATH = Path("var/data/vision_realtime_face_lock_status.json")

CONFIRM_ENV_NAME = "CONFIRM_NEXA_REALTIME_FACE_LOCK"
CONFIRM_VALUE = "RUN_REALTIME_FACE_LOCK"
...
class FaceBox:
    ...
    def clamp(self, frame_width: int, frame_height: int) -> "FaceBox":
        ...
        return FaceBox(left=left, top=top, right=right, bottom=bottom, source=self.source, score=self.score)

    def to_dict(self, *, frame_width: int, frame_height: int) -> dict[str, Any]:
        return {
            "box": {"left": self.left, "top": self.top, "right": self.right, "bottom": self.bottom},
...
class RealtimePanTiltSession:
    ...
    def _prepare_once(self) -> None:
        ...
        sequence = [
            ...
            ({"T": 4, "cmd": 2}, 0.07),
            ({"T": 210, "cmd": 1}, 0.08),
            (self._move_command(self.pan_angle, self.tilt_angle), 0.05),
        ]
    ...
    def _clamp_pan(self, value: float) -> float:
        return max(float(self.safe_limits["pan_min_degrees"]), min(float(self.safe_limits["pan_max_degrees"]), float(value)))

    def _clamp_tilt(self, value: float) -> float:
        return max(float(self.safe_limits["tilt_min_degrees"]), min(float(self.safe_limits["tilt_max_degrees"]), float(value)))
    ...
    def move_delta(self, *, pan_delta: float, tilt_delta: float) -> dict[str, Any]:
        ...
        if abs(applied_pan) < self.min_command_delta_degrees and abs(applied_tilt) < self.min_command_delta_degrees:
            return {
                "ok": True,
    ...
    def status(self) -> dict[str, Any]:
        return {
            "backend": "waveshare_realtime_face_lock_session",
...
def _validate_args(args: argparse.Namespace) -> None:
    ...
    if args.target_fps < 1.0 or args.target_fps > 40.0:
        raise SystemExit("Refusing target FPS outside 1..40.")
    ...
    if args.command_interval_seconds < 0.015:
        raise SystemExit("Refusing command interval below 0.015 seconds.")
    ...
    try:
        ...
        with RealtimePanTiltSession(
            ...
            try:
                    ...
                    try:
                        ...
                        if face is None:
                            ...
                            if face is not None:
                                ...
                                previous_points = _features_for_box(gray, face, max_corners=int(args.max_flow_corners))
                ...
                print("\nCTRL+C received. Stopping realtime face lock loop safely.")
...
def print_summary(payload: dict[str, Any]) -> None:
    print()
    print("NEXA Vision Runtime — REALTIME face lock pan-tilt")
    print(f"execute={payload['execute']}")
    print(f"preview_only={payload['preview_only']}")
    print(f"continuous_until_ctrl_c={payload.get('continuous_until_ctrl_c')}")
    print(f"interrupted_by_user={payload.get('interrupted_by_user')}")
    ...
    if payload["execute"]:
        print("HARDWARE LOOP: realtime optical-flow face lock was allowed only by explicit gates.")
    else:
        print("PREVIEW LOOP: realtime tracking ran, but hardware serial was not opened.")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run realtime optical-flow face lock with persistent pan-tilt serial session.")
    ...
    parser.add_argument("--camera-warmup-seconds", type=float, default=0.4)
    parser.add_argument("--port", default=None)
    parser.add_argument("--baudrate", type=int, default=None)
    parser.add_argument("--timeout-seconds", type=float, default=0.02)
    parser.add_argument("--serial-warmup-seconds", type=float, default=0.15)
    parser.add_argument("--max-step-degrees", type=float, default=0.85)
    parser.add_argument("--pan-gain-degrees", type=float, default=12.0)
    parser.add_argument("--tilt-gain-degrees", type=float, default=14.0)
    ...
    parser.add_argument("--dead-zone-x", type=float, default=0.006)
    parser.add_argument("--dead-zone-y", type=float, default=0.006)
    parser.add_argument("--smoothing-alpha", type=float, default=0.92)
    ...
    parser.add_argument("--detect-every-n-frames", type=int, default=18)
    parser.add_argument("--refresh-flow-every-n-frames", type=int, default=4)
    parser.add_argument("--max-flow-corners", type=int, default=90)
    parser.add_argument("--min-flow-points", type=int, default=8)
    parser.add_argument("--command-interval-seconds", type=float, default=0.022)
    parser.add_argument("--min-command-delta-degrees", type=float, default=0.015)
    parser.add_argument("--speed", type=int, default=100)
    parser.add_argument("--acceleration", type=int, default=100)
    parser.add_argument("--print-every-n-frames", type=int, default=8)

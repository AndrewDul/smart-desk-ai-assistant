...
from modules.runtime_contracts import (
    EntityValue,
    IntentMatch,
    RouteDecision,
    RouteKind,
...
class SemanticRouter:
    """
    Next-generation semantic router for NeXa.

    Goals:
    - keep the current working IntentParser alive
    - expose a cleaner routing contract for the new architecture
    - distinguish between action, conversation, mixed, and unclear
    - prepare the project for streaming-first dialogue and future vision input
    - avoid a giant phrase-only command box

    Important design rule:
    inferred tool suggestions from emotional / conversational input are added as
    non-executing suggestions, not as automatic actions.
    """
    ...
    def route(self, text: str, preferred_language: str | None = None) -> RouteDecision:
        ...
        match_text = self._normalize_for_matching(raw_text)
    ...
    def _should_treat_parser_action_as_explicit(
        ...
        *,
        ...
        action = str(parser_result.action or "").strip()
        ...
        if action == "help":
            if any(re.search(pattern, normalized_text) for pattern in self._EXPLICIT_HELP_PATTERNS):
                return True
            return False

        if action == "focus_start":
            if parser_result.data.get("minutes") is not None:
                return True
    ...
    def _build_intent_matches(
        ...
        if parser_result.action and parser_result.action not in {"unknown", "unclear"}:
            matches.append(
                IntentMatch(
                    name=parser_result.action,
                    confidence=float(parser_result.confidence),
                    entities=self._entities_from_parser_data(parser_result.data),
                    requires_clarification=bool(parser_result.needs_confirmation),
                    metadata={
                        "source": "intent_parser",
                        "normalized_text": parser_result.normalized_text,
                        "suggestions": list(parser_result.suggestions),
                    },
                )
            )
    ...
    def _entities_from_parser_data(self, data: dict[str, Any]) -> list[EntityValue]:
    ...
    def _build_explicit_tool_invocations(self, parser_result: IntentResult) -> list[ToolInvocation]:
        action = str(parser_result.action or "").strip()
        ...
        payload = dict(parser_result.data or {})
        ...
        return [
            ToolInvocation(
                ...
                confidence=max(float(parser_result.confidence), 0.75),
    ...
    def _detect_conversation_topics(self, normalized_text: str) -> list[str]:
        ...
        # Stable ordering gives more predictable primary_intent.
        unique = list(dict.fromkeys(found))
        unique.sort(key=lambda item: self._TOPIC_PRIORITY.get(item, 0), reverse=True)
        return unique

    def _looks_like_general_question(self, raw_text: str, normalized_text: str, language: str) -> bool:
    ...
    def _normalize_for_matching(text: str) -> str:
        ...
        lowered = re.sub(r"[^\w\s'-]", " ", lowered, flags=re.IGNORECASE)

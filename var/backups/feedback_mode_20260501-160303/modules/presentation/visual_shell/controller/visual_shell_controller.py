...
class VisualShellController:
    ...
    def hide_feedback(self, *, source: str = "nexa-runtime") -> bool:
        return self.send_command(
            VisualCommand(command=VisualCommandName.HIDE_FEEDBACK, payload={}, source=source)
        )

    def feedback_log_append(self, *, level: str, message: str, ts_ms: int = 0,
                            source: str = "nexa-runtime") -> bool:
        return self.send_command(
            VisualCommand(
                command=VisualCommandName.FEEDBACK_LOG_APPEND,
                payload={"level": str(level or "info"), "message": str(message or ""), "ts_ms": int(ts_ms)},
                source=source,
            )
        )
    ...
    def feedback_vision_frame(self, *, jpeg_b64: str, width: int, height: int,
                              source: str = "nexa-runtime") -> bool:
        return self.send_command(
            VisualCommand(
                command=VisualCommandName.FEEDBACK_VISION_FRAME,
                payload={"jpeg_b64": str(jpeg_b64 or ""), "width": int(width), "height": int(height)},
                source=source,
            )
        )

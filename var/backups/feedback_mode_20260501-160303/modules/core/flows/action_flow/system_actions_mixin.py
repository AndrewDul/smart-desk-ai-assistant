...
class ActionSystemActionsMixin:
    ...
    def _handle_feedback_on(self, *, route, language, payload, resolved):
        del route, payload
        assistant = getattr(self, "assistant", None)
        lane = getattr(getattr(assistant, "fast_command_lane", None), "visual_shell_lane", None) \
               or getattr(assistant, "visual_shell_lane", None)
        if lane is not None:
            ctrl = getattr(lane, "_controller", None)
            if callable(ctrl):
                try:
                    ctrl().show_feedback(language=language, source="nexa-feedback")
                except Exception as e:
                    self.LOGGER.warning("Feedback show failed: %s", e)
        spoken = self._localized(language, "Włączam tryb feedback.", "Feedback mode on.")
        return self._deliver_simple_action_response(
            language=language, action="feedback_on", spoken_text=spoken,
            display_title=self._localized(language, "FEEDBACK", "FEEDBACK"),
            display_lines=self._localized_lines(language, ["uruchomiono"], ["enabled"]),
            extra_metadata={"resolved_source": resolved.source},
        )

    def _handle_feedback_off(self, *, route, language, payload, resolved):
        del route, payload
        assistant = getattr(self, "assistant", None)
        lane = getattr(getattr(assistant, "fast_command_lane", None), "visual_shell_lane", None) \
               or getattr(assistant, "visual_shell_lane", None)
        if lane is not None:
            ...
            if callable(ctrl):
                try:
                    ctrl().hide_feedback(source="nexa-feedback")
                except Exception as e:
                    self.LOGGER.warning("Feedback hide failed: %s", e)
        spoken = self._localized(language, "Wyłączam tryb feedback.", "Feedback mode off.")
        return self._deliver_simple_action_response(
            language=language, action="feedback_off", spoken_text=spoken,
            display_title=self._localized(language, "FEEDBACK", "FEEDBACK"),
            display_lines=self._localized_lines(language, ["zamknięto"], ["closed"]),
            extra_metadata={"resolved_source": resolved.source},
        )
